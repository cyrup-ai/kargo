...
fn test_parse_package_name_only() {
    let spec = PackageSpec::parse("tokio")
        .expect("TEST FAILURE: Failed to parse valid package name 'tokio'");
...
fn test_parse_package_with_version() {
    let spec = PackageSpec::parse("tokio@1.28.0")
        .expect("TEST FAILURE: Failed to parse valid package spec 'tokio@1.28.0'");
    assert_eq!(spec.name, "tokio");
...
fn test_version_spec() {
    let spec1 = PackageSpec::parse("tokio")
        .expect("TEST FAILURE: Failed to parse 'tokio' for version spec test");
    assert_eq!(spec1.version_spec(), "\"*\"");

    let spec2 = PackageSpec::parse("tokio@1.28.0")
        .expect("TEST FAILURE: Failed to parse 'tokio@1.28.0' for version spec test");
...
fn test_output_filenames() {
    let spec1 = PackageSpec::parse("tokio")
        .expect("TEST FAILURE: Failed to parse 'tokio' for filename test");
    assert_eq!(spec1.json_filename(), "tokio.json");
    assert_eq!(spec1.markdown_filename(), "tokio.md");

    let spec2 = PackageSpec::parse("tokio@1.28.0")
        .expect("TEST FAILURE: Failed to parse 'tokio@1.28.0' for filename test");
