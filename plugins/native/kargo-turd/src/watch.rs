...
pub async fn run_watch_mode(config: &Config) -> anyhow::Result<()> {
    ...
    // Initialize watchexec with action handler (SYNC closure)
    // Note: Watchexec internally uses tokio::spawn which requires being in a tokio runtime
    // When running as a plugin (cdylib), we spawn a task to ensure runtime context is available
    // Citation: ./tmp/watchexec/crates/lib/examples/only_events.rs:6-20
    let wx = tokio::task::spawn(async move {
