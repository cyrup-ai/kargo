...
fn list_installed_plugins() -> Result<()> {
    ...
    for org_entry in std::fs::read_dir(&plugins_dir)? {
        ...
        if org_name == "local" {
            // Local plugin structure: local/{name}/{version}
            for name_entry in std::fs::read_dir(org_entry.path())? {
            ...
            // GitHub plugin structure: {org}/{repo}/{branch}/{name}/{version}
...
async fn list_remote_plugins(remote_url: &str) -> Result<()> {
    ...
    git::clone_repository(remote_url, temp.path(), None).await?;
...
fn scan_for_plugins(dir: &Path) -> Result<()> {
    ...
    for entry in jwalk::WalkDir::new(dir)
        ...
        .process_read_dir(|_depth, _path, _state, children| {
            // Prevent descending into directories we want to skip
            children.iter_mut().for_each(|child_result| {
                if let Ok(child) = child_result
                    && child.file_type.is_dir()
                    && let Some(name) = child.file_name.to_str()
                    && SKIP_DIRS.contains(&name)
                {
                    child.read_children_path = None;  // Don't descend!
                }
            });
        })
        .into_iter()
    {
        ...
        // Check for plugin Cargo.toml files
