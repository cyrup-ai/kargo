...
impl CargoWriter {
    ...
    fn update_dependency_in_table(&self, table: &mut toml_edit::Table, name: &str, version: &str) -> Result<()> {
                ...
                Item::Table(dep_table) => {
                    ...
                    if let Some(ver_item) = dep_table.get_mut("version") {
                        if let Some(ver_value) = ver_item.as_value_mut() {
                            if ver_value.is_str() {
                                *ver_value = Value::String(toml_edit::Formatted::new(version.to_string()));
                            }
                        }
                    }
                },
