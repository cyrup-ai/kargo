...
static TABLE_VERSION_PATTERN: Lazy<String> = Lazy::new(|| r#"({}.*?version\s*=\s*)["']{}["']"#.to_string());
...
impl DependencyWriter for RustScriptWriter {
    fn apply_updates(&self, source: &mut DependencySource, updates: &[DependencyUpdate]) -> Result<()> {
        match source {
                ...
                for update in updates {
                    match &update.dependency.location {
                        DependencyLocation::RustScriptCargo { section_range } => {
                            ...
                            updated_content.replace_range(section_range.0..section_range.1, &updated_section);
                        },
                        DependencyLocation::RustScriptDeps { line_range } => {
                            ...
                            updated_content.replace_range(line_range.0..line_range.1, &updated_line);
                        },
...
impl RustScriptWriter {
    ...
    fn update_version_in_cargo_section(&self, content: &mut String, name: &str, from_version: &str, to_version: &str) -> Result<()> {
        ...
        *content = simple_regex.replace_all(content, &format!("${{1}}\"{}\"", to_version)).to_string();
        ...
        *content = table_regex.replace_all(content, &format!("${{1}}\"{}\"", to_version)).to_string();
        
        Ok(())
    }
    
    /// Update a version in a cargo-deps: line
    fn update_version_in_cargo_deps(&self, content: &mut String, name: &str, from_version: &str, to_version: &str) -> Result<()> {
        ...
        if !replaced {
            ...
            *content = version_regex.replace_all(content, &format!("${{1}}\"{}\"", to_version)).to_string();
        }
    ...
    fn add_version_to_bare_dep(&self, content: &mut String, name: &str, version: &str) -> Result<()> {
        ...
        if content.contains(&format!("{},", name)) {
            // name,
            *content = bare_regex.replace(content, &format!("${{1}}{}=\"{}\",", name, version)).to_string();
            ...
            *content = bare_regex.replace(content, &format!("${{1}}, {}=\"{}\"", name, version)).to_string();
            ...
            // Just name
            *content = bare_regex.replace(content, &format!("${{1}}{}=\"{}\"${{2}}", name, version)).to_string();
        }
