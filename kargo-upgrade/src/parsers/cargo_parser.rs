            ...
            DependencySource::CargoToml { content, .. } => {
                ...
                if let Some(deps) = document.get("dependencies") {
                    if let Some(deps_table) = deps.as_table() {
                        self.parse_dependencies_table(deps_table, &mut dependencies, DependencyLocation::CargoTomlDirect)?;
                    }
                }
                ...
                if let Some(deps) = document.get("dev-dependencies") {
                    if let Some(deps_table) = deps.as_table() {
                        self.parse_dependencies_table(deps_table, &mut dependencies, DependencyLocation::CargoTomlDev)?;
                    }
                }
                ...
                if let Some(deps) = document.get("build-dependencies") {
                    if let Some(deps_table) = deps.as_table() {
                        self.parse_dependencies_table(deps_table, &mut dependencies, DependencyLocation::CargoTomlBuild)?;
                    }
                }
...
impl CargoParser {
    ...
    fn parse_dependencies_table(&self, table: &Table, dependencies: &mut Vec<Dependency>, location: DependencyLocation) -> Result<()> {
