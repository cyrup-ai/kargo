...
static TABLE_DEP_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?ms)^(\w+)\s*=\s*\{(.*?)version\s*=\s*["']([^"']+)["']"#).expect("Invalid table dep regex")
});
static DEPS_WITH_VERSION_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(\w+)\s*=\s*["']([^"']+)["']"#).expect("Invalid deps with version regex")
});
static CARGO_DEPS_FORMAT_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(\w+)=["']([^"']+)["']"#).expect("Invalid cargo deps format regex")
});
static DEBUG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"([\w-]+)=?["']?([^,"']+)["']?"#).expect("Invalid debug regex")
});
static BARE_DEPS_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?:^|,)\s*(\w+)(?:\s*,|$)").expect("Invalid bare deps regex")
});
...
impl RustScriptParser {
    fn parse_cargo_sections(&self, content: &str, dependencies: &mut Vec<Dependency>, source: &DependencySource) -> Result<()> {
    ...
    fn parse_cargo_deps_line(&self, content: &str, dependencies: &mut Vec<Dependency>, source: &DependencySource) -> Result<()> {
        for captures in CARGO_DEPS_REGEX.captures_iter(content) {
            if let Some(deps_match) = captures.get(1) {
                ...
                let line_start = captures.get(0)
                    .ok_or_else(|| anyhow::anyhow!("Failed to get match start"))?
                    .start();
                let line_end = captures.get(0)
                ...
                for cap in DEPS_WITH_VERSION_REGEX.captures_iter(deps_str) {
                    ...
                    if !deps_with_version.iter().any(|d: &String| d == name) {
                        ...
                        dependencies.push(Dependency {
                            name: name.to_string(),
                            version: version.to_string(),
                            location: DependencyLocation::RustScriptCargoDeps { 
