...
impl DocGenerator {
    ...
    pub async fn generate_markdown_docs(&self, crate_path: &Path) -> Result<PathBuf> {
        ...
        let markdown = rustdoc_json_to_markdown(&json_path).await
    ...
    fn run_cargo_doc(&self, package_name: &str, crate_path: &Path) -> Result<PathBuf> {
        ...
        let json_path = crate_path.join("target/doc").join(format!("{}.json", package_name));
        
        if !json_path.exists() {
            return Err(anyhow::anyhow!("Expected JSON file not found at {}", json_path.display()));
        }
    ...
    fn get_output_path(&self, package_name: &str) -> Result<PathBuf> {
        ...
        let kb_root = env::var("KNOWLEDGE_BASE_ROOT_DIR")
            .unwrap_or_else(|_| {
