    ...
    for id in item_ids {
        if let Some(item) = data.index.get(id) {
            match &item.inner {
                ...
                ItemEnum::Constant { .. } | ItemEnum::Static(_) => grouped.constants.push(id.clone()),
    ...
    if let ItemEnum::Use(use_item) = &item.inner {
        ...
        if let Some(name) = &item.name {
            match &item.inner {
                ...
                ItemEnum::Struct(_) => output.push_str(&format!("{} Struct `{}`\n\n", heading, name)),
                ...
                ItemEnum::ExternCrate { name: crate_name, .. } => {
        ...
        ItemEnum::Function(function) => {
            if let Some(name) = &item.name {
                ...
                for (i, (param_name, param_type)) in function.sig.inputs.iter().enumerate() {
                    output.push_str(&format!("{}: {}", param_name, format_type(param_type, data)));
    ...
    for variant_id in &enum_.variants {
        if let Some(variant_item) = data.index.get(variant_id) {
            if let Some(variant_name) = &variant_item.name {
                ...
                output.push_str(&format!("{} `{}`\n\n", "#".repeat(variant_heading_level), variant_name));
